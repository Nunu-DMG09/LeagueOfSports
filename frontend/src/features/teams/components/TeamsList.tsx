    ...
    <div className="space-y-6 pb-10">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {teams.map((team) => (
            <div key={team.id_equipo} className="group relative overflow-hidden rounded-xl border border-ls-gold/20 bg-ls-surface transition-all hover:border-ls-primary hover:shadow-lg flex flex-col">
              ...
              {canManageTeams && (
                <div className="absolute top-2 right-2 z-10 flex items-center gap-1.5 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900/60 p-1.5 rounded-lg backdrop-blur-sm">
                  <button onClick={() => setEditModal({ isOpen: true, team })} className="p-1.5 bg-blue-500/20 text-blue-400 hover:bg-blue-500 hover:text-white rounded transition" title="Editar Equipo"><Edit size={16} /></button>
                  <button onClick={() => setDeleteModal({ isOpen: true, id: team.id_equipo, name: team.nombre })} className="p-1.5 bg-red-500/20 text-red-500 hover:bg-red-500 hover:text-white rounded transition" title="Eliminar Equipo"><Trash2 size={16} /></button>
                </div>
              )}
              ...
              <div className="flex h-28 sm:h-32 items-center justify-center bg-gradient-to-b from-gray-800 to-ls-surface p-4 shrink-0">
                {team.logo_url ? (
                  ...
                  <Shield size={56} className="text-ls-gold opacity-50 transition-transform group-hover:scale-110 group-hover:opacity-100" />
                )}
              </div>
              ...
              <button onClick={() => navigate(`/teams/${team.id_equipo}`)} className="w-full bg-ls-bg/50 py-3 sm:py-3.5 text-xs sm:text-sm font-bold text-gray-400 transition-colors hover:bg-ls-primary hover:text-ls-bg uppercase tracking-widest mt-auto shrink-0">
                Ver Miembros
              </button>
            </div>
          ))}
