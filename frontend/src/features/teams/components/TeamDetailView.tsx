...
export default function TeamDetailView() {
  ...
  const loadData = async () => {
    try {
      const [teamData, membersData, usersData] = await Promise.all([
        teamService.getById(Number(id)),
        teamService.getMembers(Number(id)),
        userService.getAll()
      ]);
      setTeam(teamData);
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      await teamService.addMember(Number(id), {
      ...
      loadData(); // Recargar lista de miembros
    } catch (error: any) {
      toast.error(error.response?.data?.error || 'Error al asignar miembro');
    }
  };

  if (loading) return <div className="text-center py-10 text-gray-400">Cargando Roster...</div>;
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between border-b border-ls-gold/20 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="h-16 w-16 rounded border border-ls-gold/30 bg-ls-surface p-2">
            {team.logo_url ? <img src={team.logo_url} className="h-full w-full object-contain" /> : <ShieldCheck className="h-full w-full text-ls-gold" />}
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {members.length === 0 ? (
              <p className="text-gray-500 col-span-2 italic">No hay jugadores asignados a este equipo aún.</p>
            ) : (
              members.map((member) => (
